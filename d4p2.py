...
def card_won(card: str) -> int:
    if not card:
        return 0
    winning_nos, nums_got = card.split(': ')[1].split(' | ')
    nums_got = [*filter(lambda x: x, (x for x in nums_got.strip().split(' ')))]
    wins = 0
    for no in winning_nos.strip().split(' '):
        wins += nums_got.count(no)
    if wins == 0:
        return 0
    return wins


card_copies = defaultdict(lambda: 0)
for i, c in enumerate(inp.strip('\n').split('\n'), start=1):
    num_won = card_won(c)
    card_copies[i] += 1
    for num_cards in range(1, num_won + 1):
        card_copies[i + num_cards] += card_copies[i]
